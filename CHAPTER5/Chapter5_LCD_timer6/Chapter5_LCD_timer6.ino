// Saved as Chapter5_LCD_timer6.ino, to display time lapsed in MM:SS format
...
void loop() {
  // THIS IS CODE FOR A TIMER DISPLAYED BY A 16X2 LCD IN MINUTES:SECONDS 
  ...
  lcd.setCursor(0, 0);
  // THE FOLLOWING PRINTS THE TIME IN MINUTE SINCE THE RESET OF THE ARDUINO
  // IT IS FOLLOWED BY A COLUMN : AND THEN THE SECOND SINCE THE LAST REPORTED MINUTE
  lcd.print(millis() / 1000/60); lcd.print(":");
  if (t < 10 ) {  // IF ONE ONE DIGIT FOR THE SECOND SINCE LAST MINUTE
  lcd.print(" "); lcd.print(t); // PRINT THE TIME IN SECOND
  }
  if (t >= 10) {
  lcd.print(t);  // PRINT SECONDS WITH TWO DIGITS
  }
}
